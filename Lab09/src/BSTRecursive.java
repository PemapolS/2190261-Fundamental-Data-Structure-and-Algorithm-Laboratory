...
public class BSTRecursive {
	...
	public int numNodesLessThan(BSTNode n,int v) {
		if (n == null) return 0;
		if (n.data >= v) {
			return numNodesLessThan(n.left,v);
		} 
		return 1 + numNodesLessThan(n.left,v) + numNodesLessThan(n.right,v);
	}
	...
	public int numLeavesLessThan(BSTNode n, int v) {
		if (n == null) return 0;
		if (n.data >= v) {
			return numLeavesLessThan(n.left,v);
		} 
		if (n.left != null || n.right != null) {
			return numLeavesLessThan(n.left,v) + numLeavesLessThan(n.right,v);
		}
		return 1 + numLeavesLessThan(n.left,v) + numLeavesLessThan(n.right,v);
...
//		if (n == null) return -1;
//		if (n.data == v) {
//			if (nextOf(n.left,v) <= v && nextOf(n.right,v) <= v && !(n.left == null && n.right == null)) return n.parent.data;
//		}
...
//			if (n.left.data > v || (nextOf(n.left,v)-v < n.data-v && nextOf(n.left,v)-v > 0)) return nextOf(n.left,v);
//		}
//		return n.data;
		if (n == null)
            return v;

        // If current node's value is less than or equal to v,
        // then the successor must be in the right subtree
        if (n.data <= v) {
            return nextOf(n.right, v);
        }
        // If current node's value is greater than v,
        // then the successor may be in the left subtree
        // or it may be the current node
        else {
            // If the successor exists in the left subtree, return it
            if (nextOf(n.left, v) != v) {
                return nextOf(n.left, v);
            }
            // If the successor doesn't exist in the left subtree,
